...
// 	}
	...
	if ( absolute(dx) > absolute(dy) ) {
		...
		int xpxl1 = xend;
		int ypxl1 = ipart_(yend);
		print_big_pixel(img, xpxl1, ypxl1, set_brightness(color, rfpart_(yend)*xgap));
		print_big_pixel(img, xpxl1, ypxl1+1, set_brightness(color, fpart_(yend)*xgap));
		double intery = yend + gradient;
		...
		int xpxl2 = xend;
		int ypxl2 = ipart_(yend);
		print_big_pixel(img, xpxl2, ypxl2, set_brightness(color, rfpart_(yend) * xgap));
		print_big_pixel(img, xpxl2, ypxl2 + 1, set_brightness(color, fpart_(yend) * xgap));

		int x;
		for(x=xpxl1+1; x < xpxl2; x++) {
			print_big_pixel(img, x, ipart_(intery), set_brightness(color, rfpart_(intery)));
			print_big_pixel(img, x, ipart_(intery) + 1, set_brightness(color, fpart_(intery)));
			intery += gradient;
		}
	} else {
		if ( y2 < y1 ) {
		...
		int ypxl1 = yend;
		int xpxl1 = ipart_(xend);
		print_big_pixel(img, xpxl1, ypxl1, set_brightness(color, rfpart_(xend)*ygap));
		print_big_pixel(img, xpxl1 + 1, ypxl1, set_brightness(color, fpart_(xend)*ygap));
		double interx = xend + gradient;
		...
		int ypxl2 = yend;
		int xpxl2 = ipart_(xend);
		print_big_pixel(img, xpxl2, ypxl2, set_brightness(color, rfpart_(xend) * ygap));
		print_big_pixel(img, xpxl2 + 1, ypxl2, set_brightness(color, fpart_(xend) * ygap));

		int y;
		for(y=ypxl1+1; y < ypxl2; y++) {
			print_big_pixel(img, ipart_(interx), y, set_brightness(color, rfpart_(interx)));
			print_big_pixel(img, ipart_(interx) + 1, y, set_brightness(color, fpart_(interx)));
			interx += gradient;
		}
	}
}
