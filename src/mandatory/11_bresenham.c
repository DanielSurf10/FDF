...
// 		slope_bigger_equal_1(&aux, point_0, map);
	...
	if (steep)
	{
		...
		for (x = xpxl1 ; x <=xpxl2 ; x++)
		{
			...
			mlx_put_pixel(map->mlx_image, iPartOfNumber(intersectY), x,
						set_brightness(point_0->color, rfPartOfNumber(intersectY)));
			mlx_put_pixel(map->mlx_image, iPartOfNumber(intersectY) - 1, x,
	...
	else
	{
		...
		for (x = xpxl1 ; x <=xpxl2 ; x++)
		{
			// pixel coverage is determined by fractional
			// part of y co-ordinate
			mlx_put_pixel(map->mlx_image, x, iPartOfNumber(intersectY),
						set_brightness(point_0->color, rfPartOfNumber(intersectY)));
			mlx_put_pixel(map->mlx_image, x, iPartOfNumber(intersectY) - 1,
						set_brightness(point_0->color, fPartOfNumber(intersectY)));
			intersectY += gradient;
		}
	}
